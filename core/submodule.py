...
#sch
def build_gwc_volume_shining(refimg_fea, targetimg_fea, maxdisp, num_groups):
    B, C, H, W = refimg_fea.shape
    volume = refimg_fea.new_zeros([B, num_groups, maxdisp * 2, H, W])
    for i in range(-maxdisp, maxdisp):
        if i > 0:
            volume[:, :, i + maxdisp, :, i:] = groupwise_correlation(refimg_fea[:, :, :, i:], targetimg_fea[:, :, :, :-i],num_groups)
        elif i == 0:
...
#sch
def dispartty_regression_shining(x, maxdisp):
    assert len(x.shape) == 4
...
#x:(B, C, H, W)
def single_modal_estimator_shining(x, maxdisp): 
    disp = torch.arange(-maxdisp, maxdisp, dtype=x.dtype, device=x.device).reshape(1, maxdisp * 2, 1, 1)
    index = torch.argmax(x, 1, keepdim=True)#给定维度求最大值
    
    mask = disp.repeat(x.size(0), 1, x.size(2), x.size(3)) #(B, C, H, W)
    mask2 = torch.arange(-maxdisp, maxdisp+1, dtype=x.dtype, device=x.device).reshape([1, maxdisp*2 +1, 1, 1]).repeat(x.size(0), 1, x.size(2), x.size(3))
    ...
    return torch.sum(x * disp, 1, keepdim=True)
