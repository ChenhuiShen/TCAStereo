...
def readPFM_shining(file):
    with open(file, 'rb') as f:
        ...
        width = int(f.readline().decode('utf-8').rstrip())
        height = int(f.readline().decode('utf-8').rstrip())
...
def read_gen(file_name, pil=False):
    ...
    if ext == '.png' or ext == '.jpeg' or ext == '.ppm' or ext == '.jpg' or ext == '.bmp':
        ...
        flow = readPFM_shining(file_name).astype(np.float32)
        if len(flow.shape) == 2:
            return flow
        else:
            return flow[:, :, :-1]
